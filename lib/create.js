...
module.exports = (args, options, logger) => {
  ...
  const placeholders = require(`${templatePath}/_placeholders`);
  ...
  if (fs.existsSync(`${localPath}/_placeholders.js`)) {
    shell.rm(`${localPath}/_placeholders.js`);
  }

  logger.info('Please fill the following values…');

  // Ask for variable values
  prompt.start().get(placeholders, (err, result) => {
